...
const STEPS: Step[] = [
  {
    kicker: 'THE IDEA',
    title: 'A wallpaper that ages with you.',
    body: 'Ink redraws your full 52×80 grid every week — one more square inked. iOS never lets an app change your wallpaper itself, so you set it up once.',
    visual: (by) => <PhonePreview birthYear={by} />,
  },
  {
    kicker: 'STEP 1 · SAVE',
    title: 'Save this week’s grid.',
    body: 'Grid tab → “Use as wallpaper” → Save to Photos. It lands in an album called “Ink”.',
    visual: () => <MiniCard lines={['GRID TAB', '→ USE AS WALLPAPER', '→ SAVE TO PHOTOS', 'ALBUM: “INK”']} />,
  },
  {
    kicker: 'STEP 2 · SET',
    title: 'Make it your lock screen.',
    body: 'Settings → Wallpaper → Add New Wallpaper → Photos → pick the grid from the Ink album.',
    visual: () => <MiniCard lines={['SETTINGS', '→ WALLPAPER', '→ ADD NEW WALLPAPER', '→ PHOTOS → “INK”']} />,
  },
  {
    kicker: 'STEP 3 · AUTOMATE',
    title: 'Let a Shortcut tick it weekly.',
    body: 'Shortcuts → Automation → Weekly. Add “Get Latest Photos” (album Ink, 1 photo), then “Set Wallpaper”. Turn off Ask Before Running.',
    visual: () => <MiniCard lines={['SHORTCUTS → AUTOMATION', '→ WEEKLY', '→ GET LATEST PHOTOS (INK, 1)', '→ SET WALLPAPER', 'ASK BEFORE RUNNING: OFF']} />,
  },
  {
    kicker: 'DONE',
    title: 'One square, every week.',
    body: 'Ink refreshes the saved image each week; the Shortcut applies it. Prefer zero setup? The home and lock-screen widgets tick by themselves.',
    visual: (by) => <PhonePreview birthYear={by} />,
  },
];
